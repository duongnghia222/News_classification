...
if os.path.exists('./data/contents/news_data_vietnambiz_1_2.json'):
    print("---- Continue ----")
    with open('./data/contents/news_data_vietnambiz_1_2.json', 'r', encoding='utf-8') as f:
        output = json.load(f)
else:
    output = []
loop_item(len(output), len(output) + 2000)
with open('./data/contents/news_data_vietnambiz_1_2.json', 'w', encoding='utf-8') as f:
